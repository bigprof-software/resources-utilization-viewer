...
function assignments_insert(&$error_message = '') {
	...
	$data = [
		'ProjectId' => Request::lookup('ProjectId', ''),
		'ProjectDuration' => Request::lookup('ProjectId'),
		'ResourceId' => Request::lookup('ResourceId', ''),
...
function assignments_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $separateDV = 0, $TemplateDV = '', $TemplateDVP = '') {
	...
	$filterer_ProjectId = Request::val('filterer_ProjectId');
	$filterer_ResourceId = Request::val('filterer_ResourceId');
	...
	if(is_file(__DIR__ . '/hooks/assignments.Commitment.csv')) {
		$Commitment_data = addslashes(implode('', @file(__DIR__ . '/hooks/assignments.Commitment.csv')));
		$combo_Commitment->ListItem = array_trim(explode('||', entitiesToUTF8(convertLegacyOptions($Commitment_data))));
		$combo_Commitment->ListData = $combo_Commitment->ListItem;
	} else {
		$combo_Commitment->ListItem = array_trim(explode('||', entitiesToUTF8(convertLegacyOptions("0.25;;0.50;;0.75;;1.00"))));
		$combo_Commitment->ListData = $combo_Commitment->ListItem;
	}
	...
	$combo_StartDate->MinYear = defined('assignments.StartDate.MinYear') ? constant('assignments.StartDate.MinYear') : 1900;
	$combo_StartDate->MaxYear = defined('assignments.StartDate.MaxYear') ? constant('assignments.StartDate.MaxYear') : 2100;
	...
	$combo_EndDate->MinYear = defined('assignments.EndDate.MinYear') ? constant('assignments.EndDate.MinYear') : 1900;
	$combo_EndDate->MaxYear = defined('assignments.EndDate.MaxYear') ? constant('assignments.EndDate.MaxYear') : 2100;
	...
	if($selected_id) {
		...
		$combo_Commitment->SelectedText = (isset($filterField[1]) && $filterField[1] == '5' && $filterOperator[1] == '<=>' ? $filterValue[1] : '1.00');
	}
	...
	<script>
		...
		AppGini.current_ProjectId__RAND__ = { text: "", value: "<?php echo addslashes($selected_id ? $urow['ProjectId'] : htmlspecialchars($filterer_ProjectId, ENT_QUOTES)); ?>"};
		AppGini.current_ResourceId__RAND__ = { text: "", value: "<?php echo addslashes($selected_id ? $urow['ResourceId'] : htmlspecialchars($filterer_ResourceId, ENT_QUOTES)); ?>"};
	...
	$lookup_fields = ['ProjectId' => ['projects', 'Project'], 'ResourceId' => ['resources', 'Resource'], ];
